	...
	public override function parser(pack:Dynamic):Void 
	{
		...
		if ( pack.message_type == "MsgPlayerEnterGame")
		{
			...
			if ( pack.game_state == "OpenState")
			{
				...
				if ( bcard.length !=0)
				{
					if( bcard.length == 1) Main._model._bigwin_banker_card.push(pack.cards_info.banker_card_list[0]);
					else if ( bcard.length == 2) 
					{
						Main._model._bigwin_banker_card.push(pack.cards_info.banker_card_list[0]);
						Main._model._bigwin_banker_card.push(pack.cards_info.banker_card_list[1]);
					}
				...
				if ( pcard.length !=0)
				{
					if( pcard.length == 1) Main._model._bigwin_player_card.push(pack.cards_info.player_card_list[0]);
					else if ( pcard.length == 2) 
					{
						Main._model._bigwin_player_card.push(pack.cards_info.player_card_list[0]);
						Main._model._bigwin_player_card.push(pack.cards_info.player_card_list[1]);
					}
				...
				if ( rcard.length !=0)
				{
					if( rcard.length == 1) Main._model._bigwin_river_card.push(pack.cards_info.river_card_list[0]);
					else if ( rcard.length == 2) 
					{
						Main._model._bigwin_river_card.push(pack.cards_info.river_card_list[0]);
						Main._model._bigwin_river_card.push(pack.cards_info.river_card_list[1]);
					}
					//Main._model._bigwin_player_card.push(pack.cards_info.player_card_list);
				}				
